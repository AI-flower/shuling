...
4. **返回发布结果 + 社区保存引导**
   ...
   **强制交互边界**：发布成功后的第一条回复必须只展示下面的成功提示和 A/B/C 选项，然后立即停止，等待用户下一条消息。不要默认选择 A，不要看到“推荐”就自动上传，不要在同一轮里调用 `session-recorder-skill`，也不要输出发布总结替代这个提示。只有用户在下一条消息明确回复 A/B/C 或自然语言选择后，才继续对应分支。
   ...
   **选择识别规则**：仅在用户看到上面的 A/B/C 提示后、下一条消息中选择时触发。必须支持大小写和自然语言。收到用户回复后，先归一化为小写并去除前后空格，再按下面规则判断：
