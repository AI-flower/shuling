...
#   并通过 _guard.sh 实现 already_applied/mark_applied 幂等判断。
set -e

SKILL_DIR="$(cd "$(dirname "$0")/.." && pwd)"
source "$(dirname "$0")/_guard.sh"

if already_applied "2.1.1"; then
    echo '{"status":"skipped","reason":"already_applied","version":"2.1.1"}'
    exit 0
fi
...
if [ ! -f "$DB_PATH" ]; then
    echo '{"status":"skipped","reason":"db_not_found","version":"2.1.1"}'
    exit 0
fi
...
mark_applied "2.1.1"
echo '{"status":"ok","reason":"applied","version":"2.1.1"}'
