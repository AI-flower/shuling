...
把 agent/schemas/ 下 4 份 JSON Schema 对准 runtime 目录下对应文件做校验。
默认人类可读输出；--json 机器可读；退出码给 CI/preflight 用。

用法：
  python3 agent/scripts/validate.py
      校验当前 skill 目录（SKILL_DIR 自动 = 脚本上两级，即 agent/ 父目录）

  python3 agent/scripts/validate.py --target /path/to/skill
      校验指定 target（如 ~/.codex/skills/shuling）

  python3 agent/scripts/validate.py --file foo.json --schema state
      只校一对

  python3 agent/scripts/validate.py --json
...
# v3.0: 路径相对于 agent/ 根（脚本父目录的父目录）。
# v2 兼容层：旧 target 直接用同名子目录（无 agent/ 前缀），v3 的
# _default_skill_dir() 返回 agent/，所以下面的相对路径在两种 layout 下结果一致。
_KB = "knowledge-base"
_CFG = "config"
SCHEMA_TARGETS = [
    ...
    ("state",         "state.schema.json",         f"{_CFG}/state.json",               False),
    ("profile",       "profile.schema.json",       f"{_KB}/profile.json",              False),
    ("preferences",   "preferences.schema.json",   f"{_KB}/preferences.json",          False),
    ("audit-report",  "audit-report.schema.json",  f"{_KB}/audit-*.json",              True),
]
