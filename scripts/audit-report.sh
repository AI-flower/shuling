...
#
# 读 DB 做纯统计，输出两份文件:
#   knowledge-base/audit-<YYYY-MM-DD>.json   结构化（AI 读 + schemas/audit-report.schema.json 校验）
#   knowledge-base/audit-<YYYY-MM-DD>.md     人类可读骨架（AI 后续追加归因分析）
#
# 统计维度（MVP 8 项）:
...
#
# 用法:
#   bash scripts/audit-report.sh                         # 默认最近 90 天
#   bash scripts/audit-report.sh --days 180
#   bash scripts/audit-report.sh --output /path/out.md
#   bash scripts/audit-report.sh --json-only             # 不写 md，只输出 JSON 到 stdout
#   bash scripts/audit-report.sh --extract-patterns      # 额外输出 patterns 候选（供 AI 写 patterns.md）
#   bash scripts/audit-report.sh --include-organic       # 含 source='shuling' 的帖（默认只看 imported 老博主历史）
#
set -euo pipefail
...
[ -f "$DB_PATH" ] || { echo "ERR: DB 不存在 ($DB_PATH)。先跑 bash scripts/db.sh init" >&2; exit 1; }
