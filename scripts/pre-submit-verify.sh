...
#   bash scripts/pre-submit-verify.sh --json    # 机器可读(给 CI / agent 用)
#
# 退出码:
#   0 全部通过, 可提交社区
#   1 有断言失败
...
if [ -f "$TARGET_DB" ]; then
    check "target data/xhs.db 文件存在" 1 "$TARGET_DB"
    ...
    check "target data/xhs.db 文件存在" 0 "文件缺失"
fi
...
if [ -f "$TARGET/scripts/preflight.py" ]; then
    ...
    (cd "$TARGET" && python3 scripts/preflight.py >"$PREFLIGHT_OUT" 2>&1 || true)
